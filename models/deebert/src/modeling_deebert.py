...
sys.path.insert(1, os.path.join(os.getcwd(), './earlyexit'))
sys.path.insert(1, "../apparate/earlyexit")
...
class DeeBertEncoder(nn.Module):
    def __init__(self, config):
        ...
        # NOTE(ruipan): set_up_layers is not needed for serving in e3, since
        # EarlyExitModel handles injecting ramps
        # self.layer = None
        # self.set_up_layers(config)

    def set_up_layers(self, config):
        # TODO(ruipan): should only set up BertLayer here. Branchpoints and BertHighway should be done in summary_graph.py.
        layers = []
        for _ in range(config.num_hidden_layers):
    ...
    def init_highway_pooler(self, pooler):
        # NOTE(ruipan): should make sure we do this after activating each ramp
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            # )
            # print(f"type(layer_module) {type(layer_module)}")
            # print(f"layer_module.branched_module.attention.self.query.weight {layer_module.branched_module.attention.self.query.weight}")
            ...
            # NOTE: during serving, type(layer_module) is a regular BertLayer
            # when the exit is not active, and BranchPoint otherwise
            if type(layer_module) is BertLayer:  # no branchpoint here
                continue
            # elif type(layer_module) is BranchPoint:  # somehow not working...
            elif hasattr(layer_module, "output"):     
            ...
            if not self.training and highway_exit is not None:  # NOTE(ruipan): accommodate EarlyExitModel
                ...
                # print(f"highway_entropy {highway_entropy}")
