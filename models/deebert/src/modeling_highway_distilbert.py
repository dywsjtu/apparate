...
class DeeDistilBertTransformer(nn.Module):
    def __init__(self, config):
        ...
        self.early_exit_entropy = [-1 for _ in range(config.n_layers)]
        self.active_ramps = list(range(config.n_layers))
        # NOTE: set me to False when storing entropy
        # TODO: implement setter if args.store_entropy
        self.take_exits = False
        self.profile_computational_latency = False
    ...
    def set_early_exit_entropy(self, x):
        ...
        print(f"All ramps active, threshold list: {self.early_exit_entropy}")
    
    def set_entropy_specific_ramps(self, ramp_ids, ramp_thresholds):
        self.active_ramps = ramp_ids
        for id in ramp_ids:
            self.early_exit_entropy[id] = ramp_thresholds.pop(0)
        print(f"Certain ramps active, threshold list: {self.early_exit_entropy}")
    
    def set_profile_computational_latency(self, profile_computational_latency):
        self.profile_computational_latency = profile_computational_latency
        self.encoder_durations = []
        self.highway_durations = []
    ...
    def forward(
        ...
        all_logits = ()
        all_predictions = ()
        if self.profile_computational_latency:
            encoder_start = torch.cuda.Event(enable_timing=True)
            encoder_end = torch.cuda.Event(enable_timing=True)
            highway_start = torch.cuda.Event(enable_timing=True)
            highway_end = torch.cuda.Event(enable_timing=True)
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.profile_computational_latency:
                highway_start.record()
            highway_exit = self.highway[i](current_outputs)
            # logits, pooled_output
            if self.profile_computational_latency:
                highway_end.record()
                torch.cuda.synchronize()
                encoder_duration = encoder_start.elapsed_time(encoder_end)
                highway_duration = highway_start.elapsed_time(highway_end)
                self.encoder_durations.append(encoder_duration)
                self.highway_durations.append(highway_duration)
                # print(f"encoder_duration {encoder_duration}, highway_duration {highway_duration}")

            if not self.training:
                highway_logits = highway_exit[0]
                highway_entropy = entropy(highway_logits)
                if not self.take_exits:
                ...
                if i in self.active_ramps and highway_entropy < self.early_exit_entropy[i] \
                    and self.take_exits:
                    # new_output = (highway_logits,) + current_outputs[1:] + (all_highway_exits,)
                    new_output = (highway_logits,) + current_outputs[1:] + (None,) + (all_highway_exits,)
        ...
        if self.profile_computational_latency:
            print(f"Average encoder duration: {sum(self.encoder_durations[1:]) / len(self.encoder_durations[1:])}, \
                average highway duration: {sum(self.highway_durations[1:]) / len(self.highway_durations[1:])}")
...
class DeeDistilBertModel(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.encoder = DeeDistilBertTransformer(config)  # NOTE(ruipan): changed name for easier profiling
    ...
    def forward(
        ...
        encoder_outputs = self.encoder(  # NOTE(ruipan): changed name for consistency across models in profiling
            embedding_output,
...
class DeeDistilBertForSequenceClassification(DistilBertPreTrainedModel):
    ...
    def forward(
        ...
        # print(f"DeeDistilBertForSequenceClassification:loss {outputs[0]}")
        # print(f"DeeDistilBertForSequenceClassification:logits {outputs[1]}")
        # print(f"DeeDistilBertForSequenceClassification;hidden_states {outputs[2]}")  # FIXME: hidden_states is different
        # print(f"DeeDistilBertForSequenceClassification:attentions {outputs[3]}")  # FIXME: attentions is partially different (the first element is the same)
        # print(f"DeeDistilBertForSequenceClassification:highway_exits {outputs[4]}")
